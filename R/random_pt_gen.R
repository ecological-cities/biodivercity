...
#'
#'@export
random_pt_gen <- function(x, area_per_pt, pt_buffer, excess_modifier = 1,
    ...
    if (!is.null(forest)) {
        forest <- sf::st_read(forest) %>%
            sf::st_combine()
        boundaries_all <- boundaries
        boundaries <- sf::st_difference(boundaries_all, forest)
        forest <- sf::st_intersection(boundaries_all, forest)
        rm(boundaries_all)
        ...
        if (!is.null(retain) & !is.null(retain_prop)) {
            ...
            points <- sf::st_sample(boundaries, ceiling((n - retain_n) *
                ...
                dplyr::mutate(class = "Urban")
            points_forest <- sf::st_sample(forest, ceiling((n_forest - retain_n_forest) *
                excess_modifier), type = "random") %>%
                sf::st_as_sf() %>%
                dplyr::rename(geometry = x) %>%
                dplyr::mutate(id = dplyr::row_number()) %>%
                dplyr::mutate(id = paste0("N", id)) %>%
                rbind(., retain_forest[sample(1:nrow(retain_forest), retain_n_forest),
                  ]) %>%
                dplyr::mutate(class = "Forest")

            cat("\n", glue::glue("Number of urban points retained: {retain_n} (n = {n}, retain_prop = {retain_prop})."))
            cat("\n", glue::glue("Number of forest points retained: {retain_n_forest} (n = {n_forest}, retain_prop = {retain_prop})."))
