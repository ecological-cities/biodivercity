...
random_pt_gen <- function(x, area_per_pt, pt_buffer, excess_modifier = 1,
    natveg = NULL, retain = NULL, retain_prop = NULL) {
    ...
    if (!is.null(natveg)) {
        ...
        points <- sf::st_sample(boundaries, ceiling(n * excess_modifier),
            ...
            dplyr::mutate(id = dplyr::row_number()) %>%
            dplyr::mutate(id = paste0("U", id))
        points_NV <- sf::st_sample(natveg, ceiling(n_NV * excess_modifier),
            type = "random") %>%
            sf::st_as_sf() %>%
        ...
        if (!is.null(retain) & !is.null(retain_prop)) {
            ...
            points <- sf::st_sample(boundaries, ceiling((n - retain_n) *
                ...
                dplyr::mutate(class = "Urban")
            points_NV <- sf::st_sample(natveg, ceiling((n_NV - retain_n_NV) *
                excess_modifier), type = "random") %>%
                sf::st_as_sf() %>%
                dplyr::rename(geometry = x) %>%
                dplyr::mutate(id = dplyr::row_number()) %>%
