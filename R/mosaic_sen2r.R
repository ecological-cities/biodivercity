...
#'
#'@export
mosaic_sen2r <- function(parent_dir, exclude = NULL, rm_outlier = TRUE){ #parent_dir is the directory path as specified in sen2r `path_out` argument
  ...
  for(i in seq_along(exclude)){
    if(!exclude[i] %in% indices & !exclude[i] %in% nonspectral) stop("Argument 'exclude' does not contain any sen2r output formats." )
  }
...
# loop to process/export mosaic for each subdirectory
  ...
  for(i in 1:length(directories)){ # normal loop
          ...
          if(rm_outlier == TRUE){
            if(basename(directories[i]) %in% c("NDVI", "NDRE", "NDVIre", "GNDVI", "EVI", "ARVI", "SAVI", "OSAVI", "MSAVI2",
                                               "MTVI2", "TCIdx", "MCARI", "NDWI", "NDWI2", "NDFI", "NDFI2", "NDII", "NDBI")){
