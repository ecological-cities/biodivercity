...
This article details various ways to download and process landscape data to build predictive models of animal diversity. For example, the landscape data may be derived from open-source maps, publicly-available satellite data, or manually generated sources (e.g., on-site mapping, design scenarios). 
...
The Punggol (`PG`) area (town) in Singapore from the example data `sampling_areas` will be used as demonstration in this article.
...
```{r load example dataset of osm elements, echo = FALSE}
filepath <- system.file("extdata", "osm_data.Rdata", package = "biodivercity")
load(filepath)
```
...
We can generate summary metrics for the OSM data, at point locations where animal data are available (e.g., animal surveys have been conducted). In this example, we randomly generate three points within the `sampling_areas`, and visualise the datasets in a interactive map:
...
For example, calculate building metrics up to `100` and `200` metres away from `points`. This returns a list, with each element corresponding to a particular buffer radius. Column names include a prefix indicating that values are from OpenStreetMap (`osm_`).
