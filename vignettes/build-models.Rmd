...
<<<<<<< HEAD
knitr::kable(bestmodels_info, caption = paste0("**Summary of best models ranked based on automated model selection from `MuMIn::dredge()` (ΔAIC < 2).**")) %>%
=======
...
>>>>>>> 2c0e4d544ad3b31be80450808aa903f8ddfdf215
...
# load("../inst/extdata/build-models_beta-diversity.Rdata")
# load("../data/sampling_points.rda")

# Afrer package release:
filepath <- system.file("extdata", "build-models_beta-diversity.Rdata", package="biodivercity") # summarised landscape information per point
load(filepath)
...
```{r diagnosis by PCA}
pca_birds <- birds %>% 
  dplyr::select(any_of(landscape$var)) %>% #select impt variables from RF
...
These are the required model inputs for the predictive _Beta_-diversity model.
1. pca_birds
2. raw_pcnm_birds
3. birds.rda.fin
4. p_hats_birds
