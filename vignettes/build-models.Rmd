...
```{r show landscape data used in analyses, echo = FALSE}
...
```{r remove points with few surveys, include = FALSE, cache = TRUE}
...
- **`r length(unique(removed$point_id))`** sampling points were removed (`r nrow(removed[removed$priority == "Aves",])` Aves; `r nrow(removed[removed$priority == "Lepidoptera",])` Lepidoptera; `r nrow(removed[removed$priority == "Odonata",])` Odonata; `r nrow(removed[removed$priority == "Amphibia",])` Amphibia)   

- **`r length(unique(points_tally6$point_id))`** sampling points remained  (`r nrow(points_tally6[points_tally6$priority == "Aves",])` Aves; `r nrow(points_tally6[points_tally6$priority == "Lepidoptera",])` Lepidoptera; `r nrow(points_tally6[points_tally6$priority == "Odonata",])` Odonata; `r nrow(points_tally6[points_tally6$priority == "Amphibia",])` Amphibia) 
...
```{r read filtered survey data, include = FALSE, eval = params$update_inputdata}
...
```{r calculate sp richnness for each animal group, include = FALSE, eval = params$update_inputdata}
...
```{r append landscape data to each animal group & scale vars, include = FALSE, eval = params$update_inputdata}
...
```{r to convert from long to wide format & remove NAs, include = FALSE, eval = params$update_inputdata}
