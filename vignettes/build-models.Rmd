---
title: "Build Models"
subtitle: "Train and select the best models that predict animal diversity from landscape data"
author: "Edwin Tan"
date: "`r Sys.Date()`"
...
In the examples below, generalised linear mixed-effects models (GLMMs) are used, since linear models are generally simpler and provide better interpretability. Do note that users may use other types of models/algorithms, or first narrow down the list of potential predictor variables. For example, random forest models may be used to select variables/features based on their relative importance in improving model performance (Arthur et al., 2010; Li et al., 2017). 
...
# join to the bird data, then scale and centre predictors
birds <- birds %>% 
  inner_join(landscape) %>% 
  mutate(across(.cols = where(is.numeric) & !sprich, scale))

head(birds) # view combined data
```
...
# get predictor names
predictors <- grep("^r[0-9]+m_",  colnames(birds), value = TRUE)
...
model_null <- 
  glmer(sprich ~ 1 + (1|town), family = "poisson", data = birds)
```

Automated model selection is performed using the function `MuMIn::dredge()`. Models are ranked based on their `AICc` value (AIC adjusted for small sample sizes). Model constraints are specified in the argument `subset = subset_exp`. To avoid a long runtime in this demonstration, we also limit the number of predictor variables in each model to four (`m.lim = c(NA,4)`).
...
Finally, we can extract the list of best model object(s) for future use:
...
```{r, include = FALSE}
rm(birds, predictors, subset_exp,
   model_glm, model_global, model_null, 
   effects, importances,
   bestmodels, bestmodels_info)
```
