...
subtitle: "Comparing local diversity between sampled areas"
author: "Author"
date: "`r Sys.Date()`"
...
butterflies_tampines_round_2 <- filter_obs(observations = animal_observations, survey_ref = animal_surveys,
...
plot_sac <- 
  foreach(i = seq_along(animal_observations), 
          .packages = "tidyverse") %dopar% {
  
  unique <- animal_surveys[[i]] %>%
    distinct(town, round, priority)
  ...
  for(j in 1:nrow(unique)){
    ...
    sac_data <- sac_extractor(observations = animal_observations[[i]], 
                              survey_ref = animal_surveys[[i]],
                              specify_town = unique$town[j], 
                              specify_round = unique$round[j], 
                              specify_priority = unique$priority[j])
