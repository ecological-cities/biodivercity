...
We use `sen2r`, it can be installed from [CRAN](https://cran.r-project.org/package=sen2r).
...
for(i in 1:length(rounds)){
  thresholds[[i]] <- threshold_otsu(files[i], levels = 256)
  rm(i)
}
...
```{r ndvi histograms, include = FALSE, eval = FALSE}

plot(plots[[1]], col = rgb(178/255,223/255,138/255, 0.5), xlim = c(-1, 1), xlab = "NDVI", main = "", yaxt = 'n')
plot(plots[[2]], col = rgb(51/255,160/255,44/255, 0.5), xlim = c(-1, 1), xlab = "NDVI", main = "", yaxt = 'n', add = TRUE)
abline(v = thresholds[[1]], col="black", lwd=2, lty = 2)
abline(v = thresholds[[2]], col="black", lwd=2)
abline(v = thresholds2[[1]], col="black", lwd=2, lty = 2)
abline(v = thresholds2[[2]], col="black", lwd=2)
...
```{r eval = FALSE}
rm(veg_classified, urban_classified, water_classified,
   lsm_perpoint)
